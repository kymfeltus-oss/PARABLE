...
- Name: streamers-hub.spec.ts >> Streamers Hub — creator tools (/streamer-hub) >> broadcast mode opens Message & Study at The Table
- Location: tests\streamers-hub.spec.ts:125:7
...
Call log:
  - waiting for locator('aside').filter({ hasText: 'Broadcast mode' }).locator('button').filter({ hasText: 'Message & Study' })
