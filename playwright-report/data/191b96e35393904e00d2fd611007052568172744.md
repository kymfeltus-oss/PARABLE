...
- Name: streamers-hub.spec.ts >> Streamers Hub — creator tools (/streamer-hub) >> sermon checker shortcut opens full page
- Location: tests\streamers-hub.spec.ts:112:7
...
Call log:
  - waiting for locator('aside').filter({ hasText: 'Creator shortcuts' }).getByRole('button', { name: /sermon checker/i })
