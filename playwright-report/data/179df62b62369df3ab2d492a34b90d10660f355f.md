...
- Name: streamers-hub.spec.ts >> Streamers Hub — creator tools (/streamer-hub) >> broadcast mode opens Live Interaction in Sanctuary
- Location: tests\streamers-hub.spec.ts:136:7
...
Call log:
  - waiting for locator('aside').filter({ hasText: 'Broadcast mode' }).locator('button').filter({ hasText: 'Live Interaction' })
