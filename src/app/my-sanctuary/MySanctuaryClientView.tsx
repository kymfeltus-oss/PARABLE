...
export default function MySanctuaryClientView({
  ...
  useEffect(() => {
    // #region agent log
    debugSessionLog(
      "MySanctuaryClientView:avatar",
      "avatar resolution",
      {
        isDemoProfile,
        isOwnProfile,
        preferAuthAvatar,
        profileAvatar: profile?.avatar_url?.slice(0, 80) ?? null,
        authAvatar: avatarUrl?.slice(0, 80) ?? null,
        resolved: displayAvatar?.slice(0, 80) ?? null,
        runId: "post-fix-avatar",
      },
      "H-avatar-auth",
    );
    // #endregion
