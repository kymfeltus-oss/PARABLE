...
export default function LiveStudioPage() {
  ...
  const startLive = async () => {
    try {
      ...
      const roomName = unifiedStreamRoomName(user.id);
      setLkRoom(roomName);
      ...
      const res = await fetch("/api/livekit/token", {
        ...
        body: JSON.stringify({ room: roomName, roomName }),
      });
      ...
      const { error: liveErr } = await setProfileLiveStatus(supabase, user.id, true);
      if (liveErr) {
  ...
  const endLive = () => {
    void (async () => {
      ...
      if (user) {
        await setProfileLiveStatus(supabase, user.id, false);
      }
    })();
    setIsLive(false);
    setLkToken(null);
