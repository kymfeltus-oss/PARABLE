...
import { revalidateMySanctuaryLayout } from "@/app/my-sanctuary/actions";

export type MediaPickerMode = "image" | "video" | "any";

export type SanctuaryPostComposerHandle = {
  openMediaPicker: (mode?: MediaPickerMode) => void;
};

type SanctuaryPostComposerProps = {
  /** Instagram home–style slim row: avatar + caption cue + media + share. */
  variant?: "default" | "igHome" | "profileDock";
  /** Sets `posts.category` when inserting (requires `category` column in DB). */
  category?: string | null;
};

const PICKER_ACCEPT: Record<MediaPickerMode, string> = {
  image: "image/*",
  video: "video/*",
  any: "image/*,video/*",
};
 ...
 */
const SanctuaryPostComposer = forwardRef<SanctuaryPostComposerHandle, SanctuaryPostComposerProps>(
  function SanctuaryPostComposer({ variant = "default", category: postCategory = null }, ref) {
    const router = useRouter();
    const supabase = createClient();
    const { userProfile, avatarUrl } = useAuth();
    const userId = userProfile?.id as string | undefined;
    const showIg = variant === "igHome";
    const showProfileDock = variant === "profileDock";
    ...
    const [pickerAccept, setPickerAccept] = useState(PICKER_ACCEPT.any);
    const mediaInputRef = useRef<HTMLInputElement>(null);

    useImperativeHandle(ref, () => ({
      openMediaPicker(mode: MediaPickerMode = "any") {
        setPickerAccept(PICKER_ACCEPT[mode]);
    ...
    const handlePublish = async () => {
      ...
      try {
        ...
        clearMedia();
        setMessage("Shared.");
        await revalidateMySanctuaryLayout();
    ...
    if (showProfileDock) {
      ...
      return (
        <section className="profile-composer-dock" aria-label="New post composer">
          {hiddenInput}
          {mediaPreview ? (
            <div className="profile-composer-preview">
              {mediaFile?.type.startsWith("video/") ? (
                <video src={mediaPreview} controls className="profile-composer-preview-media" />
              ) : (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={mediaPreview} alt="" className="profile-composer-preview-media" />
              )}
            </div>
          ) : null}
          <textarea
            ...
            rows={2}
            className="profile-composer-caption"
            placeholder="Write a caption…"
          />
          <div className="profile-composer-actions">
            {mediaPreview ? (
              <button type="button" onClick={clearMedia} className="profile-composer-remove">
                Remove media
              </button>
            ) : (
              <span />
            ...
            <button
              ...
              className="profile-composer-share"
            >
              {posting ? (
          ...
          {message ? <p className="profile-composer-message">{message}</p> : null}
        </section>
      );
    }
    ...
    if (showIg) {
      return (
        <section className="ig-composer mb-0 border-b border-[#dbdbdb] bg-white px-3 py-3 sm:px-4">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full border border-[#dbdbdb] bg-[#efefef]">
              {composerAvatarSrc ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                ...
                <div className="flex h-full w-full items-center justify-center text-[10px] font-bold text-[#8e8e8e]">
            ...
            <textarea
              ...
              onChange={(e) => setCaption(e.target.value)}
              rows={1}
              className="max-h-24 min-h-[2.5rem] flex-1 resize-none bg-transparent text-[14px] text-[#262626] placeholder:text-[#8e8e8e] outline-none"
            ...
            <button
              ...
              className="mt-0.5 shrink-0 rounded-lg p-2 text-[#8e8e8e] transition hover:bg-[#fafafa] hover:text-[#00f2ff]"
          ...
          {mediaPreview ? (
            <div className="mt-3 overflow-hidden rounded-lg border border-[#dbdbdb] bg-[#fafafa]">
              {mediaFile?.type.startsWith("video/") ? (
          ...
          <div className="mt-3 flex items-center justify-between gap-2">
            {mediaPreview ? (
              <button type="button" onClick={clearMedia} className="text-[12px] font-medium text-[#8e8e8e] hover:text-[#262626]">
                Remove media
              </button>
            ) : (
              <span />
            ...
            <button
              ...
              onClick={() => void handlePublish()}
              disabled={posting || !mediaFile}
              className="rounded-lg bg-[#00f2ff] px-4 py-1.5 text-[12px] font-semibold text-[#010101] disabled:opacity-40"
            >
              {posting ? (
                <span className="inline-flex items-center gap-1.5">
...
export default SanctuaryPostComposer;
