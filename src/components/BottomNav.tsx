...
export default function BottomNav() {
  ...
  const authUserId =
    typeof userProfile?.id === "string" && userProfile.id.trim() ? userProfile.id.trim() : null;
  const items = getMainNavItems(authUserId);
        ...
        >
          ...
          <nav className={["relative grid w-full gap-x-0 gap-y-1 px-0.5 py-1.5 sm:px-1 sm:py-2", colClass].join(" ")}>
            {items.map((item) => {
              ...
              return (
                <Link key={item.label} href={item.href} className="flex min-w-0 flex-col items-center gap-0.5 sm:gap-1">
                  <IconWrap
                    active={active}
                    compact
                    studyAi={IS_STUDY_AI}
